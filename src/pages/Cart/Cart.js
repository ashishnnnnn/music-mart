...
import { Horizontal_card, OrderModal } from "../../Components";
...
export const Cart = () => {
  ...
  return (
    <>
      {showModal ? (
        <OrderModal setShowModal={setShowModal} />
      ) : (
          ...
          <div className="cart flex-center-row gap-1 ali-st">
            ...
            {user_data.cart.length > 0 && (
              <div className="pad-1 width-30  price-details flex-column mar-t-1 gap-0-8">
                ...
                <button
                  onClick={() => {
                    setUser_Data({ type: "RESET_CART" });
                    setShowModal(!showModal);
                  }}
                  className="purchase pad-0-5 fnt-1-2 flex-center-row cursor-pointer"
                >
                  Place Order
                </button>
              </div>
